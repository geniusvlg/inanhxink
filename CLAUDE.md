...
1. **Order site** (`inanhxink.com`) — React SPA where customers choose a template, pick a `qr_name`, customise content, and submit an order.
...
```
Browser → nginx (:443)
  ├── inanhxink.com               → /var/www/order (React static) + /api/ → backend:3001
